...
export const getWishlists = function (schema, request) {
	const userId = requiresAuth(this, request);
	if (!userId) {
		return new Response(404, {}, { errors: ["Invalid Credentials!"] });
	}
	const userWishlists = schema.users.findBy({ _id: userId }).wishlist;
	return new Response(200, {}, { wishlist: userWishlists });
};
...
export const addToWishlist = function (schema, request) {
	...
	try {
		...
		this.db.users.update({ _id: userId }, { wishlist: userWishlists });
		return new Response(200, {}, { wishlist: userWishlists });
	} catch (error) {
...
export const removeFromWishlist = function (schema, request) {
	const userId = requiresAuth.call(this, request);
	try {
		if (!userId) {
			return new Response(404, {}, { errors: ["Invalid Credentials!"] });
